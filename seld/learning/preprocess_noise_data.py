...
class PreprocessorNoise:
    ...
    def __init__(self, args, cfg):
        ...
        self.data_dir = Path(cfg['dataset_noise_dir'])
        #self.data_dir = dataset.dataset_dir[args.dataset_type]['esc50']
        data_h5_dir = hdf5_dir.joinpath('noise_dataset').joinpath('{}fs'.format(cfg['data']['sample_rate']))

        self.data_h5_dir_list = data_h5_dir.joinpath('esc50')

        self.data_statistics_path = data_h5_dir.joinpath('statistics_noise.txt')
    ...
    def extract_data(self):
        ...
        """
        print('Converting noise wav file to hdf5 file starts......\n')
        if self.data_h5_dir_list.is_dir():
            flag = input("HDF5 folder {} is already existed, delete it? (y/n)".format(self.data_h5_dir_list)).lower()
            if flag == 'y':
                shutil.rmtree(self.data_h5_dir_list)
            elif flag == 'n':
                print("User select not to remove the HDF5 folder {}. The process will quit.\n".format(self.data_h5_dir_list))
                return
        self.data_h5_dir_list.mkdir(parents=True)

        #for idx, data_dir in enumerate(self.data_dir_list):
        begin_time = timer()
        h5_dir = self.data_h5_dir_list
        #statistic_path = self.data_statistics_path
        audio_count = 0
        silent_audio_count = 0
        self.audio_dir = self.data_dir.joinpath('audio')
        data_list = [path for path in sorted(self.audio_dir.glob('5-*.wav')) if not path.name.startswith('.')]
        iterator = tqdm(data_list, total=len(data_list), unit='it')
        for data_path in iterator:
        # read data - sampling like the original data.
            data, _ = librosa.load(data_path, sr=self.cfg['data']['sample_rate'], mono=True)
            # save to h5py
        ...
        iterator.close()
        print("Extacting of ESC50 feature finished! Time spent: {:.3f} s".format(timer() - begin_time))
