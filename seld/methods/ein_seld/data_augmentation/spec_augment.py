...
# exper 1: W=5, F=30, T=40, num_freq_masks=2, num_time_masks=2, replace_with_zero=False
# exper 2: None, F=30, T=40, num_freq_masks=2, num_time_masks=2, replace_with_zero=False
# exper 3: None, F=30, T=100, num_freq_masks=2, num_time_masks=2, replace_with_zero=False
# exper 4: W=2, F=30, T=100, num_freq_masks=2, num_time_masks=2, replace_with_zero=False
# exper 5: W=None, F=8, T=8, num_freq_masks=2, num_time_masks=6, replace_with_zero=False
def specaug(
    spec, W=2, F=8, T=8, num_freq_masks=2, num_time_masks=6, replace_with_zero=False
):
    """SpecAugment
    ...
    return combined_spec.permute(0,2,1)
...
def time_warp(spec, W=5):
    ...
    num_channels = 4
    device = spec.device
    # the 4 channel spectrogram
    spec_channels = torch.empty(num_channels,spec.size()[1], spec.size()[2], device=device)

    for n in range(num_channels):
        spec_c = spec[n, :, :]
        spec_c = spec_c.unsqueeze(0)
        spec_len = spec_c.shape[1]
        num_rows = spec_c.shape[2]  # shape (batch_size, num_mel_bins,time_steps)
        y = num_rows // 2
        horizontal_line_at_ctr = spec_c[:, :, y][0]
        assert len(horizontal_line_at_ctr) == spec_len
        point_to_warp = spec_c[:, random.randrange(W, spec_len - W), y]
        #point_to_warp = horizontal_line_at_ctr[:,random.randrange(W, spec_len - W)]
        assert isinstance(point_to_warp, torch.Tensor)

        # Uniform distribution from (0,W) with chance to be up to W negative
        dist_to_warp = random.randrange(-W, W)
        #src_pts_l, dest_pts_l = (
        #    [torch.tensor([[[point_to_warp[t], y]]], device=device) for t in range(len(point_to_warp))],
        #    [torch.tensor([[[point_to_warp[t] + dist_to_warp, y]]], device=device) for t in range(len(point_to_warp))])
        #src_pts , dest_pts = torch.empty(32,src_pts_l[0].size()[1],src_pts_l[0].size()[2],device=device) , \
        #                     torch.empty(32,dest_pts_l[0].size()[1],dest_pts_l[0].size()[2],device=device)
        #for sr,des in zip(src_pts_l, dest_pts_l):
        #    torch.cat((src_pts, sr), 0)
        #    torch.cat((dest_pts,des),0)
        src_pts, dest_pts = torch.tensor([[[point_to_warp, y]]], device=device) , torch.tensor([[[point_to_warp + dist_to_warp, y]]], device=device)
        warped_spectro, dense_flows = sparse_image_warp(spec_c, src_pts, dest_pts)
        #warped_spectro = warped_spectro.squeeze(3).squeeze(0).permute(1,0)
        #warped_spectro = warped_spectro.permute(2, 1)
        spec_channels[n,:,:] = warped_spectro.squeeze(3).squeeze(0)
            #torch.cat((spec_channels, ), dim=0)
...
def freq_mask(spec, F=30, num_masks=1, replace_with_zero=False):
    ...
    for n in range(num_channels):
        spec_c = spec[n, :, :]
        cloned = spec_c.unsqueeze(0).clone()
        ...
        for i in range(0, num_masks):
            ...
            if f_zero == f_zero + f:
                spec_channels[n, :, :] = cloned.squeeze(0)
...
def solve_interpolation(train_points, train_values, order, regularization_weight):
    ...
    #ones = torch.cat((c, torch.ones(b, dtype=train_points.dtype, device=device).view([-1, 1, 1])), 2).float()
    #ones = torch.ones(b, dtype=train_points.dtype, device=device).view([-1, 1, 1])
    ...
    #torch.cat((matrix_a.squeeze(0), torch.transpose(matrix_b, 2, 1)), 1)
    #torch.cat((matrix_a, torch.transpose(matrix_b, 2, 1)), 1)
...
def cross_squared_distance_matrix(x, y):
    ...
    x_y_transpose = torch.matmul(x.squeeze(0), y.squeeze(0).transpose(0,1))
        #torch.matmul(x.squeeze(0), y.squeeze(0).transpose(0, 1))
...
def interpolate_bilinear(
    ...
    def gather(y_coords, x_coords, name):
        ...
        gathered_values = torch.reshape(linear_coordinates,[1,linear_coordinates.shape[0]*linear_coordinates.shape[1]])
