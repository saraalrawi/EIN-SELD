...
class EINV2(nn.Module):
    def __init__(self, cfg, dataset):
        ...
        #self.attention_layer_sed_1 = nn.Sequential(AttentionModule(inplanes=64, planes=64)),

        self.sed_conv_block2 = nn.Sequential(
            DoubleConv(in_channels=64, out_channels=128),
            nn.AvgPool2d(kernel_size=(2, 2)),
        )
        #self.attention_layer_sed_2 = nn.Sequential(AttentionModule(inplanes=128, planes=128)),

        self.sed_conv_block3 = nn.Sequential(
            DoubleConv(in_channels=128, out_channels=256),
            nn.AvgPool2d(kernel_size=(1, 2)),
        )
        #self.attention_layer_sed_3 = nn.Sequential(AttentionModule(inplanes=256, planes=256)),
        ...
        #self.attention_layer_doa_1 = nn.Sequential(AttentionModule(inplanes=64, planes=64)),

        self.doa_conv_block2 = nn.Sequential(
            DoubleConv(in_channels=64, out_channels=128),
            nn.AvgPool2d(kernel_size=(2, 2)),
        )
        #self.attention_layer_doa_2 = nn.Sequential(AttentionModule(inplanes=128, planes=128)),

        self.doa_conv_block3 = nn.Sequential(
            DoubleConv(in_channels=128, out_channels=256),
            nn.AvgPool2d(kernel_size=(1, 2)),
        )
        #self.attention_layer_doa_3 = nn.Sequential(AttentionModule(inplanes=256, planes=256)),
