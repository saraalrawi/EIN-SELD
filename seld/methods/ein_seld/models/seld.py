...
class EINV2(nn.Module):
    ...
    def forward(self, x):
        ...
        #x_doa_attention_1 = self.attention_layer_doa_1[0].cuda()(x_doa_feat_1)
        ...
        #x_sed_attention_2 = self.attention_layer_sed_2[0].cuda()(x_sed_feat_2)

        x_doa_feat_2 = self.doa_conv_block2(x_doa_feat_1)

        #x_doa_attention_2 = self.attention_layer_doa_2[0].cuda()(x_doa_feat_2)
        ...
        #x_sed_attention_3 = self.attention_layer_sed_3[0].cuda()(x_sed_feat_3)
        x_doa_feat_3 = self.doa_conv_block3(x_doa_feat_2)
        #x_doa_attention_3 = self.attention_layer_doa_3[0].cuda()(x_doa_feat_3)
...
class SELD_ATT(nn.Module):
    def __init__(self, cfg, dataset):
        ...
        self.shared_conv_block5 = nn.Sequential(nn.Conv2d(in_channels=512,
                                                          out_channels=1024,
                                                          kernel_size= 1,
                                                          padding=0,
                                                          bias=False),
        ...
        self.shared_network = [self.shared_conv_block1, self.shared_conv_block2, self.shared_conv_block3, self.shared_conv_block4, self.shared_conv_block5]
        ...
        if self.pe_enable:
            self.pe = PositionalEncoding(pos_len=100, d_model=512, pe_type='t', dropout=0.0)
        self.sed_trans_track1 = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=1024, nhead=8, dim_feedforward=1024, dropout=0.2), num_layers=2)
        self.sed_trans_track2 = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=1024, nhead=8, dim_feedforward=1024, dropout=0.2), num_layers=2)
        self.doa_trans_track1 = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=1024, nhead=8, dim_feedforward=1024, dropout=0.2), num_layers=2)
    ...
    def forward(self, x):
        ...
        shared_feature = [[0]*7 for _ in range(5)]
        ...
        for i in range(2): # iterate over the tasks
            for j in range(5): # iterate over the shared feature space
                if j == 0:
                    # a
        ...
        #x_sed = atten_encoder[0][-1][-1].mean(dim=3)   # (N, C, T)
        #x_doa = atten_encoder[1][-1][-1].mean(dim=3)  # (N, C, T)

        x_sed = atten_encoder[0][4][2].mean(dim=3)
        x_doa = atten_encoder[1][4][2].mean(dim=3)
